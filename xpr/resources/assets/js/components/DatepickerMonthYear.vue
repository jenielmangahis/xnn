...
<script>
    export default {
        ...
        mounted: function () {
            ...
            jQuery(this.$el)
                ._datepicker({...this.options, format: "yyyy-mm", minViewMode: 1,})
                ._datepicker('setStartDate', this.toDate(this.startDate))
                .on('changeDate',  (e) => {
                    vm.$emit('input', moment(e.date).format("YYYY-MM"));
                });
        ...
        methods: {
            toDate(date) {
                ...
                return moment(date, "YYYY-MM").toDate();
            },
        },
        watch: {
            options(options) {
                jQuery(this.$el)._datepicker('destroy');
                jQuery(this.$el).empty()._datepicker({...this.options, format: "yyyy-mm", minViewMode: 1,});
            },
            startDate(startDate) {
