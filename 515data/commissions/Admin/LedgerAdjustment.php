...
class LedgerAdjustment
{
    ...
    public function saveAdjustment($user_id, $description, $amount, $type, $created_by)
    {
        ...
        $adjustment = DB::transaction(function () use ($user_id, $description, $amount, $type, $created_by) {
            ...
            $commission_period = CommissionPeriod::whereRaw("CURRENT_DATE() BETWEEN start_date AND end_date")
                ->ofType(config("commission.commission-types.other-income"))
                ->first();

            if ($commission_period === null) {
                throw new \Exception('Commission period not found.');
            }

            $payout = new CommissionPayout();
            $payout->commission_period_id = $commission_period->id;
            $payout->transaction_id = 0;
            $payout->user_id = $user_id;
            $payout->sponsor_id = $user_id;
            $payout->payee_id = $user_id;
            $payout->level = 0;
            $payout->commission_value = $amount;
            $payout->amount = $amount;
            $payout->percent = 100;
            $payout->remarks = "Ledger Adjustment: " . $description;
            $payout->is_paid = 0;
            $payout->save();

            $lp = new LedgerPayout();
            $lp->ledger_id = $ledger->id;
            $lp->payout_id = $payout->id;
            $lp->save();
    ...
    public function deleteAdjustment($id, $deleted_by)
    {
        $undoAdjustment = DB::transaction(function () use ($id, $deleted_by) {
            ...
            $payout->commission_period_id = $commission_period->id;
            $payout->transaction_id = 0;
            $payout->user_id = $undoAdjustment->user_id;
            $payout->sponsor_id = $undoAdjustment->user_id;
            $payout->payee_id = $undoAdjustment->user_id;
            $payout->level = 0;
            $payout->commission_value = $undoAdjustment->amount * -1;
            $payout->amount = $undoAdjustment->amount * -1;
            $payout->percent = 100;
            $payout->remarks = "Ledger Adjustment: " . $undoAdjustment->description . " (UNDO ADJUSTMENT)";
            $payout->is_paid = 0;
            $payout->save();
