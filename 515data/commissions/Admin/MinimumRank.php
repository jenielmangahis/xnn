    ...
    public function getMinimumRanks($filters)
    {
        ...
        $query = DB::table('cm_minimum_ranks AS mr')
            ->selectRaw("
                ...
                mr.user_id,
                mr.rank_id,
                DATE_FORMAT(mr.start_date,'%d/%m/%Y') AS start_date,
                DATE_FORMAT(mr.end_date,'%d/%m/%Y') AS end_date,
                mr.created_by_id,
                DATE_FORMAT(mr.updated_at,'%d/%m/%Y %H:%i:%s') AS updated_at,
                r.name AS minimum_rank,
                CONCAT(u.fname, ' ', u.lname) member,
                IF(c.id IS NOT NULL, CONCAT(c.fname, ' ', c.lname), NULL) created_by,
                r1.name AS paid_as_rank,
                r2.name AS rank_name
            ")
            ->join('cm_ranks AS r', 'r.id', '=', 'mr.rank_id')
            ->join('users AS u', 'u.id', '=', 'mr.user_id')
            ->leftJoin('users AS c', 'c.id', '=', 'mr.created_by_id')
            ->join('cm_daily_ranks AS cdr', function($join){
                $join->on('cdr.user_id', '=' ,'mr.user_id');
                $join->on('cdr.rank_date', '=', DB::raw('CURRENT_DATE()'));
            })
            ->join('cm_ranks AS r1', 'r1.id', '=', 'cdr.paid_as_rank_id')
            ->join('cm_ranks AS r2', 'r2.id', '=', 'cdr.rank_id')
