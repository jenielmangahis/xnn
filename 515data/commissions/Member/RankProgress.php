    ...
    public function getProgress($filters, $user_id = null)
    {
        ...
        $query = DailyVolume::join("cm_daily_ranks AS dr", "dr.volume_id", "=", "cm_daily_volumes.id")
            ...
            ->selectRaw("
                ...
                cr.id AS current_rank_id,
                cr.mar_limit as mar,
                dr.paid_as_rank_id,
                pr.name AS paid_as_rank,
                dr.is_active,
                cm_daily_volumes.pea,
                cm_daily_volumes.ta - cm_daily_volumes.pea as ta,
                cm_daily_volumes.qta,
                cm_daily_volumes.level - $level AS level,
                u.sponsorid AS sponsor_id,
                IF(s.active = 'Canceled', 'Terminated Associate', CONCAT(s.fname, ' ', s.lname)) AS sponsor,
                dr.rank_date
            ")
            ->whereRaw("cm_daily_volumes.volume_date = CURRENT_DATE() AND u.active <> 'Canceled'");
        ...
        if(!!$user_id) {
            $query->whereRaw("EXISTS(
                WITH RECURSIVE downline (user_id, parent_id, `level`) AS (
                    ...
                    WHERE p.levelid = 3
                        -- AND downline.level < 2 /* Client requested to show all. */
                )
        ...
        if ($is_all_below) {
            ...
            $query->where('dr.rank_id', '<=', $rank_id);
        } else {
            $query->where('dr.rank_id', $rank_id);
        }
...
//        }
        ...
        $nextRank = Rank::find($rank_id);

        $data->map(function ($volume, $key) use ($nextRank) {
            $volume->needs = VolumesAndRanks::getNextRankRequirementsByDailyVolume($volume, $nextRank);
            return $volume;
        });
