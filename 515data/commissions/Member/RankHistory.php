...
class RankHistory
{
    ...
    const ENROLLMENT_RANK_FILTER = "career_title_id";
    ...
    protected function getEnrollmentQuery($user_id, $start_date, $rank_id, &$level = 0)
    {
        ...
        $query =
            ...
            ->join("cm_ranks AS cr", "cr.id", "=", "dr.rank_id")
            ->join("cm_ranks AS pr", "pr.id", "=", "dr.career_title_id")
            ->join("cm_ranks AS pdr", "pdr.id", "=", "dr.paid_as_rank_id")
            ...
            ->selectRaw("
                ...
                cr.name AS current_rank,
                dr.paid_as_rank_id,
                pdr.name AS paid_as_rank,
                pr.name AS career_title,
                dv.pea,
                dv.ta - dv.pea AS ta,
                dv.mar,
                dv.qta,
        ...
        if (!!$rank_id && static::ENROLLMENT_RANK_FILTER === "career_title_id") {
            $query->where('dr.career_title_id', $rank_id);
        } elseif (!!$rank_id && static::ENROLLMENT_RANK_FILTER === "paid_as_rank_id") {
            $query->where('dr.paid_as_rank_id', $rank_id);
        }
