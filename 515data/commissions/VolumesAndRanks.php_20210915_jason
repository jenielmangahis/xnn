...
final class VolumesAndRanks extends Console
{
    ...
    private function getRank($user_id, $pea, $qta)
    {
		...
		if ($pea >= 400 && $qta >= 7000) { //founding leader
			...
			//sum up all ranks
            $nationalLeader = +$c->get($r['national-leader']);
            $globalLeader = +$c->get($r['global-leader']);
            $presidentLeader = +$c->get($r['president-leader']);
            $ceoLeader = +$c->get($r['ceo-leader']);
            $foundingLeader = +$c->get($r['founding-leader']);

			$biggerRankLegCount = $globalLeader + $presidentLeader + $ceoLeader + $foundingLeader;
			$smallerRankLegCount = ($biggerRankLegCount - 2) + $nationalLeader;

			if ($biggerRankLegCount >= 2 && $smallerRankLegCount >= 2) {
				return 12;
			}
		}
		...
		if ($pea >= 250 && $qta >= 3500) { //ceo leader
			...
			if ($biggerRankLegCount >= 2 && $smallerRankLegCount >= 1) {
				return 11;
			}
		} 
		...
		if ($pea >= 150 && $qta >= 1500) { //president leader
			...
			$eligibleLegCount = $nationalLeader + $globalLeader + $presidentLeader + $ceoLeader + $foundingLeader;

			if ($eligibleLegCount >= 2) {
				return 10;
			}
		} 
		...
		if ($pea >= 100 && $qta >= 700) { //global leader
			...
			$eligibleLegCount = $groupLeader + $nationalLeader + $globalLeader + $presidentLeader + $ceoLeader + $foundingLeader;
		...
		if ($pea >= 75 && $qta >= 350) { //national leader
			...
			$eligibleLegCount = $teamLeader + $groupLeader + $nationalLeader + $globalLeader + $presidentLeader + $ceoLeader + $foundingLeader;
		...
		if ($pea >= 50 && $qta >= 150) { //group leader
			...
			$eligibleLegCount = $watt60 + $teamLeader + $groupLeader + $nationalLeader + $globalLeader + $presidentLeader + $ceoLeader + $foundingLeader;
		...
		if ($pea >= 40 && $qta >= 80) {
			...
			$eligibleLegCount = $watt30 + $watt60 + $teamLeader + $groupLeader + $nationalLeader + $globalLeader + $presidentLeader + $ceoLeader + $foundingLeader;
    ...
    public static function getNextRankRequirementsByDailyVolume(DailyVolume $volume, Rank $nextRank)
    {
        ...
        //MAR
        $mar = $nextRank->mar_limit ? $nextRank->mar_limit : 0;

        $watt_60 = config('commission.ranks.watt-60');

        if($nextRank->id <= $watt_60) {

            $requiredPea = $nextRank->pea - $pea;
            $requiredTa = $nextRank->ta_requirement - $qta;
    ...
    public static function getCurrentRankRequirementsByDailyVolume(DailyVolume $volume, Rank $currentRank)
    {
        if ($currentRank === null || $volume === null) return [];
        $needs = [];

        $user_id = $volume->user_id;
        $current_rank_id = +$currentRank->id >= 6 ? +$currentRank->id - 2 : +$currentRank->id; //minus 2 ranks every leg requirement from rank 6 - 10

        $rank_count = Rank::countLegRequirement($current_rank_id, $user_id);
        $rank_count =  2 - +$rank_count; // leg requirement - leg count
        ...
        //MAR
        $mar = $currentRank->mar_limit ? $currentRank->mar_limit : 0;

        $watt_60 = config('commission.ranks.watt-60');

        if($currentRank->id <= $watt_60) {
            
            if($currentRank->pea > 0) {
		...
		if (+$currentRank->id === 6) {
            ...
            $needs[] = [
                'label' => 'Leg Requirements',
                'value' => '2 National Leaders or higher',
                'description' => ''
            ];
