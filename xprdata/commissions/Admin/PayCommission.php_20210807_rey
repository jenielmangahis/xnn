        ...
        $history = DB::transaction(function() use ($payout_ids, $prepared_by_id, $period_ids, $history) {
            ...
            try
            {
                ...
                foreach($payouts as $payout)
                {
                    ...
                    $payout_ids = explode(',', $payout->ids);
                    $this->logger($history->id,"          ", false);
                    $this->logger($history->id, "Sending \${$payout->amount} payment to {$payout->name}");
                    ...
                    $this->logger($history->id,"Successfully sent to {$payout->name}. Transaction No. {$payment->transaction_no}");

                    if(self::DEBUG) $this->logger($history->id,"Marking as paid: " . implode(",", $payout_ids));

                    Payout::whereIn('id', $payout_ids)->update([
