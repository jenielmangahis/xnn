...
class TransactionsReport
{
     ...
     private function getQuery($start_date, $end_date, $status = "All")
     {
          ...
          if(in_array($status, ['Approved', 'Declined', 'Error', 'Failed'])) {
               ...
               if($status == 'Approved') {
                    $sql .= " AND EXISTS(SELECT 1 FROM v_cm_transactions tt WHERE tt.transaction_id = a.id AND DATE(tt.transactiondate) BETWEEN '$start_date' AND '$end_date')";
               }
          }
     ...
     private function getLineItemQuery($start_date,$end_date,$status="All")
     {
          ...
          if(in_array($status, ['Approved', 'Declined', 'Error', 'Failed'])) {
               ...
               if($status == 'Approved') {
                    $sql .= " AND EXISTS(SELECT 1 FROM v_cm_transactions tt WHERE tt.transaction_id = t.id AND DATE(tt.transactiondate) BETWEEN '$start_date' AND '$end_date')";
               }
          }
