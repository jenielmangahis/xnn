...
class NachaFile
{
    ...
    private function createFileHeader_old()
    {
        $this->fileHeader = '101 ' . $this->formatText($this->bankrt, 10, true) . $this->formatText($this->fileId, 10, true) . ($this->is_test ? (date('ymd') . "TEST") : date('ymdHi')) . $this->filemodifier . $this->recordsize . $this->blockingfactor . $this->formatcode . $this->formatText($this->originatingBank, 23) . $this->formatText($this->companyName, 23) . $this->formatText($this->referencecode, 8);
    ...
    private function formatText($txt, $spaces, $is_right_justified = false)
    {
        return substr(str_pad(strtoupper($txt), $spaces, ' ', $is_right_justified ? STR_PAD_LEFT : STR_PAD_RIGHT), 0, $spaces);
    }
    ...
    public function generateFilename($filename)
    {
        ...
        $file = storage_path("app/ach/$filename");

        if (!file_put_contents($file, $this->fileContents)) {
