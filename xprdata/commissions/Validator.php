 ...
 * @version     1.5
 */
class Validator // Renamed from GUMP
{
    ...
    protected $defaultMessages = array(
        ...
        'validate_exists'                   => 'The {field} does not exists in {param}',
        'validate_array'                   => 'The {field} field must be an array',
    );
...
//        }
    ...
    public static function sanitizeThenValidate(array $data, array $validators) // Added by Vienzent
    {
        $gump = self::get_instance();
        ...
        if ($gump->run($data) === false) {
            throw new \Exception($gump->get_readable_errors(true)); // TODO: create class for this exception
        } else {
            return $data;
        }
    }
    ...
    protected function validate_exists($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }
        ...
        $count= DB::table($table)->where($column, $input[$field])->count(DB::raw("1"));
