    ...
    public function getParentDetails($user_id)
    {
        ...
        $sql = "
            SELECT
                ...
                IFNULL(
                    r.name, IF(EXISTS(SELECT 1 FROM categorymap cm WHERE cm.userid = u.id AND FIND_IN_SET(cm.catid, '$customers')), 
                        IF(
                ...
                IFNULL(ROUND(dv.group_volume_left_leg, 2), 0) AS left_leg,
                IFNULL(ROUND(dv.group_volume_right_leg, 2), 0) AS right_leg,
                EXISTS(
                    SELECT 1
                    FROM v_cm_transactions t 
                    WHERE t.user_id = u.id 
                        AND t.type = 'product'
                        AND DATE(t.transaction_date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY) AND CURRENT_DATE()
