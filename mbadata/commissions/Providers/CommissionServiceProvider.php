...
use Commissions\Payments\Payeer;
...
class CommissionServiceProvider extends ServiceProvider
{
    ...
    protected function registerPaymentService()
    {
        $this->app->bind(PaymentInterface::class, function ($app) {
            ...
            switch (config('commission.payment')) {
                ...
                case 'payeer':
                    return new \Commissions\Payments\Payeer($app->make(\Commissions\Clients\Payeer::class));
                default:
    ...
    protected function registerPayeerClient()
    {
        $this->app->bind(\Commissions\Clients\Payeer::class, function ($app) {

            return new PayeerClient(
                config('services.payeer.account_number'),
                config('services.payeer.client_id'),
                config('services.payeer.client_password'),
                config('services.payeer.api_url')
            );
