...
class Clawback
{
    ...
    public function refundOrder($data)
    {
        ...
        if (isset($data['is_full_order']) && $data['is_full_order']) {
            $data['percent'] = 0;
            $data['amount'] = 0;
        } else if (isset($data['amount']) && $data['amount']) {
            $data['percent'] = 0;
            $data['is_full_order'] = 0;
        } else if (isset($data['percent']) && $data['percent']) {
            $data['amount'] = 0;
            $data['is_full_order'] = 0;
        }
        ...
        if (+$data['percent'] === 0 && +$data['is_full_order'] === 0 && +$data['amount'] === 0) {
            throw new \Exception("Please select any of the options in Refund Order by: ('% Off' OR 'Amount Off' OR 'Full Order')");
        }
        ...
        $clawback = DB::transaction(function () use ($clawback, $data, $transaction) {
            ...
            foreach ($data['products'] as $product)
            {
                ...
                if (+$clawbackProduct->quantity > $transaction_product->quantity) {
                    throw new \Exception('Refund Quantity should be lesser than the total item quantity');
                }
