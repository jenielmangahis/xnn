...
//        $memberId = $request['memberId'];
        ...
        $query = DB::table('cm_move_invoice_logs as l')
            ->selectRaw("
                ...
                l.old_user_id
            ")
            ->leftjoin('users AS u', 'u.id', '=', 'l.new_user_id')
            ->leftjoin('users AS s', 's.id', '=', 'l.new_sponsor_id')
            ->leftjoin('users AS ou', 'ou.id', '=', 'l.old_user_id')
            ->leftjoin('users AS os', 'os.id', '=', 'l.old_sponsor_id')
            ->leftjoin('users AS m', 'm.id', '=', 'l.changed_by_id')
            ->leftjoin('transactions AS t', 't.id', '=', 'l.transaction_id');
