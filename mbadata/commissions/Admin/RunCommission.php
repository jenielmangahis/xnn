...
use App\RankIncentives As Incentives;
...
use Commissions\CommissionTypes\CarBonus;
use Commissions\CommissionTypes\BinaryCommission;
use Commissions\CommissionTypes\RankIncentives;
    ...
    public function run($commission_period_id)
    {
        ...
        try {
            ...
            $background_worker = DB::transaction(function () use ($commission_period_id, $commission_type_id) {
                ...
                $commission->setItemCount($count);
                $chunk = $commission->getChunkCount();
...
//                GiftCard::deleteByPeriod($commission_period_id);
                ...
                if($chunk_count > static::PROCESS_LIMIT) {
                    throw new AlertException("Process limit exceeded. Please increasing the chucking.");

                    /**
                     * NOTE:
                     *
                     * Giadd ni sya na checking para dili maangol ang server
                     *
                     * Para ma increase ang chunking, need nimu i override ang getChunkCount sa commission type. Ang default na value kay 50
                     *
                     * Kung daghan na jud kaau ang mga order (+2500) na gina process, pwede nimu ing.anion ang getChunkCount
                     *
...
//                return new SampleCommission($period, $background_worker_logger, $payout_repository);
            ...
            case config('commission.commission-types.car-bonus'):
                return new CarBonus($period, $background_worker_logger, $payout_repository);
            case config('commission.commission-types.binary-commission'):
                return new BinaryCommission($period, $background_worker_logger, $payout_repository);
            case config('commission.commission-types.rank-incentives'):
                return new RankIncentives($period, $background_worker_logger, $payout_repository);
            default:
                return new SampleCommission($period, $background_worker_logger, $payout_repository);
