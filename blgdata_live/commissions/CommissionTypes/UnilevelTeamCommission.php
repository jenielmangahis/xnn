...
class UnilevelTeamCommission extends CommissionType
{
    ...
    {
		//TODO: make sure na naay records sa cm_genealogy_placement or ga populate jud diri. kay if dili, masuggest nako use users table na lang
    ...
    {
        if($rank_id == config('commission.ranks.ambassador')
            || $level > self::MAX_LEVEL
        ) return 0;

		//TODO: make sure na naay value inig kuha sa value sa array
		// Ex. $rank_id = 1; $level = 5
		// $percentage[1][4] = null or not existing

        $percentage = [
            config('commission.ranks.silver-influencer')     => [2, 0, 0, 0, 0, 0, 0, 0]
            , config('commission.ranks.gold-influencer')     => [2, 3, 0, 0, 0, 0, 0, 0]
		    , config('commission.ranks.platinum-influencer') => [2, 3, 4, 0, 0, 0, 0, 0]
            , config('commission.ranks.sapphire-influencer') => [2, 3, 4, 5, 0, 0, 0, 0]
            , config('commission.ranks.pearl-influencer')    => [2, 3, 4, 5, 6, 0, 0, 0]
            , config('commission.ranks.emerald-influencer')  => [2, 3, 4, 5, 6, 0, 0, 0]
            , config('commission.ranks.ruby-influencer')     => [2, 3, 4, 5, 6, 7, 0, 0]
            , config('commission.ranks.diamond-influencer')  => [2, 3, 4, 5, 6, 7, 0, 0]
            , config('commission.ranks.double-diamond-influencer') => [2, 3, 4, 5, 6, 7, 8, 0]
            , config('commission.ranks.triple-diamond-influencer') => [2, 3, 4, 5, 6, 7, 8, 0]
