...
class Autocomplete
{
    ...
    public function getMembers($search, $page = 0)
    {
        ...
        if(is_numeric($search) && is_int(+$search)) {
            ...
            $query->where(function ($query) use ($search) {
                $query->orWhere('u.fname', 'LIKE', "%{$search}%")
                    ->orWhere('u.lname', 'LIKE', "%{$search}%")
                    ->orWhere('u.site', 'LIKE', "%{$search}%")
                    ->orWhereRaw("CONCAT('#', u.id, ': ', u.fname, ' ', u.lname) LIKE ?", ["%{$search}%"]);
            });
    ...
    public function getEnrollerCustomerDownline($member_id, $search, $page = 0)
    {
            ...
            ->whereRaw("EXISTS(
                WITH RECURSIVE downline (user_id, parent_id, `level`) AS (
                    SELECT 
                        ...
                        1 AS `level`
                    FROM users u
                    WHERE u.sponsorid = ? AND u.levelid = 3
                        AND EXISTS(SELECT 1 FROM categorymap cm WHERE cm.userid = u.id AND FIND_IN_SET(cm.catid, '$customer'))
                    ...
                    INNER JOIN downline ON p.sponsorid = downline.user_id
                    WHERE p.levelid = 3
                        AND EXISTS(SELECT 1 FROM categorymap cm WHERE cm.userid = p.id AND FIND_IN_SET(cm.catid, '$customer'))
                )
                SELECT 1 FROM downline d WHERE d.user_id = u.id
            )", [$member_id]
            )->orderBy("u.id");
