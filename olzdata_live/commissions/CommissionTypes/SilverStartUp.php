...
class SilverStartUp
{
        ...
        $u = DB::table('cm_affiliates as a')
            ->selectRaw(
                "
                    ...
                    a.affiliated_at BETWEEN au.affiliated_at AND DATE_ADD(au.affiliated_at, INTERVAL ".$num_of_days." DAY) 
                    AS is_qualified_sponsor, 
                    u.sponsorid,
                    (SELECT 
    ...
    public static function userIsQualified($user_id)
    {
        $u = DB::table('cm_affiliates as a')
            ...
            ->where('u.active', 'Yes')
            ->whereRaw("DATE_ADD(a.affiliated_at, INTERVAL ".self::FIRST_NINETY_DAYS." DAY) >= CURRENT_DATE()")
            ->where('u.id', $user_id)
            ->count();
