...
class IncentiveTool
{
    ...
    protected $today;
    protected $yesterday;
    protected $incentive_point_date_flag;
    ...
    public function __construct()
    {
        ...
        $this->incentive_point_date_flag = $this->getCorrectDate();
    }

    public function getCorrectDate()
    {
        $total = DB::table('cm_daily_incentive_points')
        ->select('user_id')
    ...
    public function getClosedIncentive($filters)
    {
        ...
        $query = DB::table('cm_incentive_tool_settings as cits')
            ...
            ->where("cits.end_date", "<", $this->today)
            ->where("cits.is_hide_closed_incentive", "=", 0);
    ...
    public function getArbitraryBonus($filters)
    {
        ...
        $query = DB::table('users AS u')
            ...
            ->where("cits.is_active", 1)
            ->where("cits.is_locked", 0)
            ->where("cdip.incentive_point_date", "=", $this->incentive_point_date_flag);
    ...
    public function getAllRepresentativeByIncentive($filters, $id, $is_active, $is_locked)
    {
        ...
        $query = DB::table('users AS u')
            ->selectRaw("
                ...
                u.id AS user_id, 
                CONCAT(u.fname, ' ', u.lname) AS `name`, 
                cdip.points AS points, 
                cdip.bonus_points as bonus_points
            ")
            ->join("cm_daily_incentive_points AS cdip", "u.id", "=", "cdip.user_id")
            ->join("cm_incentive_tool_settings AS cits", "cits.id", "=", "cdip.settings_id")
            ->where("cits.is_active", $is_active)
            ->where("cits.is_locked", $is_locked)
            ->where("cits.id", $settings_id)
            ->where("cdip.incentive_point_date", date('Y-m-d'));
    ...
    public function getRepresentatives($request)
    {
            ...
            $sql = "
                ...
                WHERE cdip.`settings_id` = :settings_id
            ";
            switch ($filter) {
                case 'id':
    ...
    {
        $data = ITS::findOrFail($id);
        $data->is_hide_closed_incentive = 1;
        $data->save();
    }
    ...
    public function deleteArbitraryPoints($id)
    {
        ...
        if(count($arbitrary) > 0) {
            ...
            $rep_points = DB::table('cm_daily_incentive_points')
                ...
                ->where('incentive_point_date', '=', $this->incentive_point_date_flag)
                ->get();
    ...
    public function addArbitraryPoints($data)
    {
        if(empty($data['user_id'])) {
            throw new \ErrorException("Please select a Member");
        }

        if(empty($data['settings_id'])) {
            throw new \ErrorException("Please select an Incentive Settings");
        }

        if(empty($data['bonus_points'])) {
            throw new \ErrorException("Please select enter Bonus Points");
        }
        ...
        $sql = "
            ...
            WHERE cdip.`incentive_point_date` = :incentive_point_date
        ...
        $stmt->bindParam(':incentive_point_date', $this->incentive_point_date_flag);
        $stmt->execute();
