...
class TransactionsReportController extends Controller 
{
    ...
    public function getAllTransactions($start_date, $end_date, Request $request)
    {
        return response()->json(
            $this->adminReportTransactions->getTransactionsDateRange($start_date, $end_date, $request->input('status'))
        );
    }
    ...
    {
        return response()->json($this->adminReportTransactions->getReportCSV($start_date, $end_date, $request->input('status')));
    }
