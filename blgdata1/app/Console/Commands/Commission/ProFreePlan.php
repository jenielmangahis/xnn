...
class ProFreePlan extends Console
{
    ...
    public function upgradeToProFreePlan($user_id)
    {
        ...
        $user = $affiliate::find($user_id);
        $user->cat_id = $pro_free_plan;
        $user->save();

        $this->log("Insert new catid in categorymap. user_id");
    ...
    public function getMembers()
    {
        ...
        $sql = "
            ...
            u.levelid = 3 
            GROUP BY u.id
            HAVING cv >= 25000 OR (SELECT 1 FROM cm_minimum_ranks mr WHERE mr.user_id = u.id AND mr.influencer_level >= 2);
        ";
