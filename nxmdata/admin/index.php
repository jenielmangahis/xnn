...
function populateGeneratedCodes(){
	...
	$.each(data, function(k, v) {
	$("#generatedCodesList").append("<li class=\"list-group-item\" id='"+ v.id +"'>"+'('+v.url+')'+v.code+" <a data-id='"+ v.id +"' style='cursor:pointer;' class='removeGenerated btn btn-warning'> Remove</a></li>");
			});
		}
...
function populateCannedEditor(){
	...
	$.each(data, function(k, v) {
	$("#cannedMsgsList").append("<li class=\"list-group-item\" id='"+ v.id +"'>"+v.msg+" <a data-id='"+ v.id +"' style='cursor:pointer;' class='removeCanned btn btn-warning'>Remove</a></li>");
			});
		}
...
function  getUsers(){
	...
	if(v.active==0){
			...
			htmlOffline+= '<button data-id="'+v.roomid+'" class="viewHistory btn btn-default btn-xs" data-id="'+v.user+'" >History</button>';
			
			htmlOffline+= '<button data-id="'+v.id+'" data-user="'+v.roomid+'" class="archiveBtn btn btn-default btn-xs" data-id="'+v.user+'" >Archive</button>';
			...
			htmlOffline+= '<button data-id="'+v.roomid+'" class="archiveBtn btn btn-default btn-xs" data-id="'+v.user+'" >Archive</button>';
		...
		html += '<button id="'+v.roomid+'" class="joinRoomBtn btn btn-default btn-xs" data-id="'+v.user+'" >Assist</button>';
		...
		html +='</li>';
			...
			if(v.id!=''){
					ctrls = '<button data-user="'+v.roomid+'" class="KickUser btn btn-default btn-xs" data-id="'+v.id+'" >End Session</button>';
