DROP FUNCTION IF EXISTS sp_getTotalCV;
DELIMITER //
CREATE FUNCTION sp_getTotalCV(
...
BEGIN
	...
	Post-condition:
		Return the sum of cv(shoppingcart_products.cv) column that are present 
		on transaction table. Given the date range and . */
	RETURN
		COALESCE(
		   (SELECT SUM(scp.cv) AS total_cv
