...
class RankProgress
{
    ...
    public function getProgress($filters, $user_id = null)
    {
        ...
        $leader_rank = config('commission.ranks.leader');
        ...
        $query = DailyVolume::join("cm_daily_ranks AS dr", "dr.volume_id", "=", "cm_daily_volumes.id")
            ...
            ->selectRaw("
                ...
                prs,
                grs,
                sponsored_qualified_representatives_count as sqr,
                sponsored_leader_or_higher_count as sl,
                COALESCE(
                    (
                    SELECT COUNT(1) FROM cm_daily_ranks rr
                    JOIN users ur on rr.user_id = ur.id 
                    WHERE rr.rank_date = CURRENT_DATE() 
                    AND ur.sponsorid = u.id
                    AND rr.rank_id >= $leader_rank
                    ),
                 0) as l1,
